              ...
              <el-tab-pane closable label="待就诊列表" name="1" title="1">
                        ...
                        <el-form :inline="true" class="demo-form-inline"> 
                          ...
                          <el-button>查询</el-button> -->
                             ...
                             <template>
                              <div>
                                <Input search enter-button="搜索" placeholder="请输入姓名" @on-search="searchByName()" v-model="patName" />
                              </div>
                               </template>
                             </el-col>
                          </el-row>
                      ...
                      <template>
                        ...
                        <template slot-scope="scope">
                          <el-button  @click="addTab(editableTabsValue,scope.row),findAllPM(scope.row.id);" ref="addTab" type="primary" plain size="small">查看</el-button>
                          <el-button  @click.native.prevent="remTab(editableTabsValue,scope.row,scope.$index, tableData)" ref="remTab" type="danger" plain  size="small">移除</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </template>
                    ...
                    <Col span="16" pull="3">
                      ...
                      <div v-if="current===0">
                                        <Divider size="small">一、基本信息</Divider>
                                            <el-form   label-width="80px" :inline="true" size="mini">
                                              ...
                                              <el-form-item label="姓名"><el-input v-model="forms[item.name-3].name"></el-input></el-form-item>
                                              <el-form-item label="身份证号"><el-input v-model="forms[item.name-3].idcard"  :disabled="true"></el-input></el-form-item>
                                              ...
                                              <el-form-item label="年龄"><el-input v-model="forms[item.name-3].age"></el-input></el-form-item>
                                              <el-form-item label="电话号"><el-input v-model="forms[item.name-3].number"></el-input></el-form-item>
                                              <el-form-item label="科室"><el-input v-model="forms[item.name-3].department" :disabled="true"></el-input></el-form-item>
                                              <el-form-item label="余额"><el-input v-model="forms[item.name-3].money" :disabled="true"></el-input></el-form-item><br>
                      ...
                      <div v-if="current===1">
                           <Divider size="small">三、病例</Divider>
                            <template>
                              <Collapse  accordion v-for="md1 in patientMdTD[item.name-3]" :key="md1.id" v-show="md1">
                                  <Panel name=md1.id>
                                    {{md1.visittime}}<Icon type="ios-medkit" />  {{md1.conclusion}}
                                    <p slot="content"><Divider size="small" ><font color="#2b85e4">医生</font></Divider><Icon type="md-person" />:{{md1.doctor}}</p>
                                    <p slot="content"><Divider  size="small"><font color="#2b85e4">状况详情</font></Divider>{{md1.symptom}}</p>
                                    <p slot="content"><Divider  size="small"><font color="#2b85e4">药物</font></Divider>{{md1.drug}}</p>
                                  </Panel>
                              </Collapse>
                          </template>  
                      ...
                      <div v-if="current===2">
                        ...
                        <Select v-model="model17" filterable allow-create >
                          <Option v-for="item in cheackTable" :value="item.name" :key="item.name">{{ item.name }}</Option>
                        </Select>
                       </Col>
                        <Col span="4">
                  ...
                  <!-- 检查 -->
                      ...
                      <div v-if="current===3">
                        <todo-item ref="conslusion" ></todo-item>
                          <center>
                            <el-button @click="endAll(forms[item.name-3].name,forms[item.name-3].age,forms[item.name-3].sex,forms[item.name-3].number,forms[item.name-3].id,forms[item.name-3].decri),removeTab">结束就诊</el-button>
                          </center>
                      </div>
...
export default {
  data() {
   ...
   methods: {
     ...
     findAllPatient(key){
            let _this=this
            Axios.get('/api/viewdoctor/findAllPatient', {  //params参数必写 , 如果没有参数传{}也可以
            params: { 
              page: key,
              size: 8,
            }
    ...
    //添加检查项
     ...
     findAllCheck(){
       var _this=this
        Axios.get("/api/viewdoctor/findAllCheck").then((response)=>{
    ...
    findAllPM(key){
     ...
     findPatienthistory(key,de){
      ...
      //  alert(de)
        Axios.get('/api/viewdoctor/findPatientByDepartment', {  //params参数必写 , 如果没有参数传{}也可以
            params: { 
              page: key,
              size: 8,
              department:de,
